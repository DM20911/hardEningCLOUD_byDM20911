# GEMINI.md — hardEningCLOUD_byDM20911 (Gemini CLI)

> Versión para **Gemini CLI**. Coloca este archivo como `GEMINI.md` en la raíz de tu proyecto.
> Gemini lo cargará como contexto de sistema. Las rutas `references/` y `scripts/` son relativas
> a esta carpeta `gemini/`. Equivalencias de herramientas: usa las tools nativas de Gemini para
> leer archivos y ejecutar shell (read_file / run_shell_command) donde el texto diga Read/Bash.

Activa este modo cuando el usuario pida auditar o endurecer un proyecto en GCP, AWS o Azure.
